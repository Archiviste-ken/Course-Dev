...
const unsubscribe = myStore.subscribe(() => {
  console.log("State updated:", myStore.getState());
...
myStore.dispatch({ type: decrement }); // dispatch is a method that is used to send an action to the store. The store will then call the reducer function with the current state and the action as arguments, and the reducer will return a new state based on the action type.

myStore.dispatch({ type: increment });

myStore.dispatch({ type: incrementBy, payload: 5 });

myStore.dispatch({ type: decrementBy, payload: 5 });

setTimeout(() => {
  myStore.dispatch({ type: decrement, payload: 5 });
}, 2000);
